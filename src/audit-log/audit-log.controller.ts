...
import { createAuditLog, deleteAuditLog, getAuditLogByResource } from './audit-log.service';
 ...
 * 取消审核
 */
export const revoke = async (request: Request, response: Response, next: NextFunction) => {
  ...
  try {
    ...
    const canRevokeAudit =
      auditLog && auditLog.status === AuditLogStatus.pending && auditLog.userId === userId;
